...
sample_anxiety_level_mean <- anxiety_level %>%
  pivot_longer(anxiety_1:anxiety_5, "Questions", values_to = "Response") %>%
  group_by(user_id) %>%
  summarise(anxiety_mean = mean (Response, na.rm = TRUE)) 

anxiety_level<-inner_join(anxiety_level, 
                          sample_anxiety_level_mean, 
                          "user_id")


anxiety_level %>%
  ggplot(aes(x = native, y = anxiety_mean, fill = native)) +
...
#Power_test_for_sample_size
library(pwr)
library(tidyverse)


group_min_sample_size <- pwr.t.test(d = .2,
           power = .8,
           sig.level = .05,
           alternative = "greater",
           type = "two.sample") %>%
